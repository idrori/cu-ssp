...
def CNN_BIGRU():
    # inp is one-hot encoded, inp_alt is the non one-hot encoded sequence
...
def run_test(_model, data1, data2, data3, csv_name, npy_name):
    ...
    for i in range(len(test_input_data)):
        res = decode_results(test_input_seqs[i], y_test_pred[i], revsere_decoder_index)
...
X_val   = [val_input_data, val_input_data_alt, val_profiles_np]
y_val   = val_target_data
...
# Run accuracy test on test dataset and save the output
run_test(model,
    test_input_data[:],
    test_input_data_alt[:],
    test_profiles_np[:],
