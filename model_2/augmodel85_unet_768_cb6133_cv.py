...
for fold_id in range(0, 10):
    ...
    train_augment_data = train_augment_data_all[training_idx]
    test_augment_data = train_augment_data_all[test_idx]

    index = np.arange(0,len(train_df))
    # np.random.seed(0)
    # np.random.shuffle(index)

    chunk_all = [int(len(train_df)/kfold)*i for i in range(1,kfold)]
    val_chunks = np.split(index, chunk_all)
    chunk = val_chunks[fold_id]

    val_df = train_df.iloc[chunk]
    train_train_df = train_df.iloc[list(set(index)-set(chunk))]
    ...
    X_val = train_input_data[chunk]
    X_train = train_input_data[list(set(index)-set(chunk))]
    y_val = train_target_data[chunk]
    y_train = train_target_data[list(set(index)-set(chunk))]

    X_train_augment = train_augment_data[list(set(index)-set(chunk))]
    X_val_augment = train_augment_data[chunk]
    ...
    log_dir = '../logs/{}'.format(model_name) + str(fold_id)
    os.mkdir(log_dir)
